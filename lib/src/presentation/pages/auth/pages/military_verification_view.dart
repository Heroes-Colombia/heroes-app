...
class _MilitaryVerificationViewState extends State<MilitaryVerificationView> {
  ...
  void initState() {
    super.initState();
    debugPrint('MilitaryVerificationView: initState called. User ID: ${widget.userId}');
  }
          ...
          children: [
            ...
            if (_currentAttempt > 1) ...[
              ...
              Container(
                ...
                child: Row(
                  children: [
                    Icon(Ionicons.information_circle, color: Colors.orange, size: 20),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Text(
        ...
        child: Column(
          ...
          children: [
            ...
            if (result.mismatches.isNotEmpty) ...[
              ...
              ...result.mismatches.map((mismatch) => Padding(
  ...
  Widget _buildCameraButton() {
    ...
    if (_currentAttempt == 1) {
      ...
      buttonText = 'Intentar Nuevamente ($_currentAttempt/${MilitaryVerificationService.maxAttempts})';
    } else {
  ...
  Future<void> _takePicture() async {
    ...
    if (!permission.isGranted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Se requiere permiso de cámara para verificar el documento'),
        ),
      );
      return;
    }
  ...
  Future<void> _processImage(XFile image) async {
    ...
    debugPrint('MilitaryVerificationView: _processImage started. Attempt: $_currentAttempt');
    ...
    try {
      ...
      debugPrint('MilitaryVerificationService returned result: ${result.status}, Score: ${result.matchScore}');
      ...
      debugPrint('MilitaryVerificationView: Error during verification in _processImage: $e');
      setState(() {
        _isProcessing = false;
      });
  ...
  Future<void> _handleVerificationResult(VerificationResult result) async {
    debugPrint('MilitaryVerificationView: _handleVerificationResult called with status: ${result.status}');
    switch (result.status) {
      case VerificationStatus.approved:
        debugPrint('MilitaryVerificationView: Status approved. Popping with result.');
        await _showSuccessDialog();
        break;
      case VerificationStatus.retryRequired:
        debugPrint('MilitaryVerificationView: Status retryRequired. Incrementing attempt.');
        setState(() {
          _currentAttempt++;
        });
        break;
      case VerificationStatus.manualReview:
        debugPrint('MilitaryVerificationView: Status manualReview. Popping with result.');
        await _showManualReviewDialog();
        break;
      case VerificationStatus.failed:
        debugPrint('MilitaryVerificationView: Status failed. Current attempt: $_currentAttempt, Max attempts: ${MilitaryVerificationService.maxAttempts}');
        if (_currentAttempt < MilitaryVerificationService.maxAttempts) {
          setState(() {
            _currentAttempt++;
          });
        } else {
          debugPrint('MilitaryVerificationView: Max attempts reached for failed status. Popping with result.');
  ...
  Future<void> _showSuccessDialog() async {
    ...
    locator.get<AppMethods>().showDialogAlert(
      ...
      "Continuar",
      () {
        debugPrint('MilitaryVerificationView: Success dialog confirmed. Popping to SignUpView.');
        AutoRouter.of(context).pop(_lastResult);
      },
    );
  }
  ...
  Future<void> _showManualReviewDialog() async {
    ...
    locator.get<AppMethods>().showDialogAlert(
      ...
      "Entendido",
      () {
        debugPrint('MilitaryVerificationView: Manual review dialog confirmed. Popping to SignUpView with result.');
        AutoRouter.of(context).pop(_lastResult);
      },
    );
  }
  ...
  void _navigateToManualReview() {
    debugPrint('MilitaryVerificationView: Navigating to manual review (popping with result).');
    AutoRouter.of(context).pop(_lastResult);
  }

  void _navigateToLogin() {
    debugPrint('MilitaryVerificationView: Navigating to login (popping with result).');
    AutoRouter.of(context).pop(_lastResult);
  }
}