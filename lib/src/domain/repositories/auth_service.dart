  ...
  */
  ...
  Future<bool> isEmailRegistered(String email) async {
    try {
      ...
      log(
        "Firebase error checking email: ${e.code} - ${e.message}, Function: isEmailRegistered, File: auth_service.dart",
      );
      // For errors, assume email is available to allow signup attempt
      // The actual signup will fail if the email truly exists
      return false;
    } catch (e) {
      log(
        "Unexpected error checking email: $e, Function: isEmailRegistered, File: auth_service.dart",
      );
      // If there's an unexpected error, assume email is available to allow signup attempt
      return false;
    }
  }
